...
public final class Fullbright extends Module {
    private final NumberSetting gamma  = register(new NumberSetting("Gamma", "Brightness level", 16, 1, 20, 0.5));
    private double savedGamma = -1;

    public Fullbright() { super("Fullbright", "Removes darkness", Category.RENDER, -1); }
    ...
    public void onTick(TickEvent e) {
        if (e.phase != TickEvent.Phase.PRE || mc.options == null) return;
        if (mc.options.getGamma().getValue() != gamma.getValue())
