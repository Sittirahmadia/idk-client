...
public final class AimAssist extends Module {
    private final NumberSetting smoothing = register(new NumberSetting("Smoothing", "Lerp speed", 0.12, 0.01, 1, 0.01));
    ...
    private final NumberSetting noise     = register(new NumberSetting("Noise",     "Jitter",      0.06, 0,    3, 0.01));
    private final BoolSetting weaponOnly  = register(new BoolSetting("Weapon Only", "Only sword/axe", false));
    private final BoolSetting sticky      = register(new BoolSetting("Sticky",      "Lock target",    true));
    private final BoolSetting losCheck    = register(new BoolSetting("LoS Check",   "Visibility check",true));

    private Entity target; double noiseY = 0, noiseP = 0;
    private final Random rng = new Random();

    public AimAssist() { super("Aim Assist", "Smooth aim assistance", Category.COMBAT, -1); }
    ...
    public void onRender(RenderHudEvent e) {
        ...
        if (target != null && (!target.isAlive()
                ...
                || !inFov(pYaw, pPitch, getAimPos(target))))
            target = null;
        ...
        if (losCheck.getValue()) {
            var ray = mc.world.raycast(new RaycastContext(
                    mc.player.getCameraPosVec(e.delta), aimPos,
        ...
        double dx = aimPos.x - eyes.x, dy = aimPos.y - eyes.y, dz = aimPos.z - eyes.z;
