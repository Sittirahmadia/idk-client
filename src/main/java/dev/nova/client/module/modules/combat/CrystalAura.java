...
public final class CrystalAura extends Module {
    ...
    private final NumberSetting minDmg  = register(new NumberSetting("Min Damage",  "Min dmg to break",   2,   0,36, 0.5));
    private final NumberSetting maxSelf = register(new NumberSetting("Max Self",    "Max self damage",    12,  0,36, 0.5));
    private final BoolSetting   antiSui = register(new BoolSetting ("Anti Suicide", "Skip if lethal",     true));
    private final BoolSetting   rotate  = register(new BoolSetting ("Rotate",       "Face crystal",       true));
    private final BoolSetting   multiHit= register(new BoolSetting ("Multi Hit",    "Hit all in range",   false));

    private int clock = 0;
    private final Random rng = new Random();

    public CrystalAura() { super("Crystal Aura", "Breaks nearby crystals", Category.COMBAT, -1); }
    ...
    public void onTick(TickEvent e) {
        ...
        for (EndCrystalEntity c : mc.world.getEntitiesByClass(EndCrystalEntity.class,
            ...
            float selfDmg = DamageUtil.crystalDamage(mc.player, c.getPos());
            if (antiSui.getValue() && selfDmg >= selfHp) continue;
            ...
            if (rotate.getValue()) {
                ...
                mc.player.setYaw((float)(Math.toDegrees(Math.atan2(dz,dx))-90+rng.nextGaussian()*0.6));
                mc.player.setPitch(MathHelper.clamp((float)(-Math.toDegrees(Math.atan2(dy,Math.sqrt(dx*dx+dz*dz)))+rng.nextGaussian()*0.4),-90,90));
            }
            mc.interactionManager.attackEntity(mc.player, c);
            mc.player.swingHand(Hand.MAIN_HAND);
            if (!multiHit.getValue()) break;
        }
    }
}
