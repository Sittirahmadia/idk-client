...
public final class Scaffold extends Module {
    private final BoolSetting  tower=register(new BoolSetting("Tower","Auto-tower when jumping",false));
    private final NumberSetting delay=register(new NumberSetting("Delay","Ticks between places",1,0,5,1));
    private int clock=0;
    public Scaffold(){super("Scaffold","Places blocks under your feet",Category.MOVEMENT,-1);}
    @EventHandler
    public void onTick(TickEvent e){
        ...
        for(int i=0;i<9;i++){
            ...
            if(s.getItem() instanceof BlockItem){InventoryUtil.switchTo(i);break;}
        }
        ...
        for(Direction d:Direction.values()){
            ...
            if(!st.isAir()&&st.isSolidBlock(mc.world,adj)){
                Vec3d hitVec=Vec3d.ofCenter(adj).add(d.getOpposite().getOffsetX()*0.5,d.getOpposite().getOffsetY()*0.5,d.getOpposite().getOffsetZ()*0.5);
                mc.interactionManager.interactBlock(mc.player,Hand.MAIN_HAND,new BlockHitResult(hitVec,d.getOpposite(),adj,false));
